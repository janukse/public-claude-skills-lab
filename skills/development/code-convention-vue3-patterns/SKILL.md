...
> 공통 품질 규칙(주석/문서화, 테스트, Git/협업)은 `code-convention-quality` 스킬을 참조하세요.
...
> 포괄적인 보안 규칙(XSS 방지, 입력 검증, 환경 변수, 민감 정보 보호, 암호학적 난수 생성)은 `code-convention-security` 스킬을 참조하세요. 아래는 Vue 3 프레임워크에 특화된 보안 규칙입니다.
...
Grep 도구로 베이스 컴포넌트 네이밍(Base/App/The 접두사) 위반을 탐지합니다:
- 패턴: `^(Base|App|The)[A-Z]`
- 경로: `<target-path>/components/base` 또는 `<target-path>/components/common`
- Glob 필터: `*.vue`
- 베이스 컴포넌트 디렉토리에 있으나 파일명이 `Base|App|The` 접두사를 따르지 않으면 위반 (G6.3)

Grep 도구로 단일 파일 다중 컴포넌트 export를 탐지합니다 (Vue는 SFC 특성상 드물지만 .ts에 정의된 컴포넌트 케이스):
- 패턴: `^export (function|const) [A-Z][A-Za-z]*Component`
- Glob 필터: `*.ts`
- 동일 파일에서 2회 이상 매칭 시 위반

### Step 6: G7 보안 검사

먼저 `code-convention-security verify`를 호출하여 G9 일반 보안 규칙(XSS, 입력 검증, 환경 변수, 민감 정보, 의존성, 메모리, 난수)을 검증합니다. 그 후 아래 Vue 3 고유 패턴을 추가 검증합니다.
...
| code-convention-vue3:G1-G3 | ... | ... | ... | ... |
| code-convention-vue3-patterns:G4. 디렉티브 | 1 | 0 | 0 | ❌ |
| code-convention-vue3-patterns:G5. Router | 0 | 1 | 0 | ⚠️ |
| code-convention-vue3-patterns:G6. 파일 구조 | 0 | 0 | 1 | ✅ |
| code-convention-vue3-patterns:G7. 보안 | 0 | 0 | 0 | ✅ |
...
| `code-convention-quality/SKILL.md` | 공통 품질 규칙 (주석/문서화, 테스트, Git/협업) |
| `code-convention-security/SKILL.md` | 공통 보안 규칙 (XSS, 입력 검증, 환경 변수, 민감 정보) |
| `vite.config.*` | Vite 설정 |
