...
| [code-convention-react-patterns](skills/development/code-convention-react-patterns/) | React 패턴 컨벤션(G4-G7). 스타일링, 성능, 파일 구조, 보안 규칙 | code-convention, code-convention-react, code-convention-security |
...
| [code-convention-vue3-patterns](skills/development/code-convention-vue3-patterns/) | Vue 3 패턴 컨벤션(G4-G7). 디렉티브, Router, 파일 구조, 보안 규칙 | code-convention, code-convention-vue3, code-convention-security |
| [code-convention-node](skills/development/code-convention-node/) | Node.js Core 컨벤션(G1-G3). API 설계, 미들웨어, DB 핵심 규칙 | code-convention |
| [code-convention-node-ops](skills/development/code-convention-node-ops/) | Node.js 운영 컨벤션(G4-G6). 설정 관리, 로깅, 보안 규칙 | code-convention, code-convention-node, code-convention-security |
| [code-convention-ios](skills/development/code-convention-ios/) | iOS(Swift + SwiftUI/UIKit) 컨벤션(G1-G6). 네이밍, 옵셔널, 함수/클로저, ARC, 에러 처리, UI 프레임워크 규칙 | - |
| [code-convention-android](skills/development/code-convention-android/) | Android(Kotlin + Jetpack Compose/View) 컨벤션(G1-G6). 네이밍, Null Safety, 람다, 코루틴, 에러 처리, UI/Lifecycle 규칙 | - |
...
| [design-workshop](skills/workflow/design-workshop/) | 범용 설계 워크플로우. 설계 유형 자동 감지, 전문가 패널 검토, 단계별 가이드, ADR/설계 명세 문서 생성, feature-planner 연계 지원 | doc-review-panel |
...
| **iOS 개발** | `code-convention-ios` |
| **Android 개발** | `code-convention-android` |
...
```
code-convention (Core, TS/JS)
├── code-convention-quality
├── code-convention-security
├── code-convention-react
│   └── code-convention-react-patterns ── code-convention-security
├── code-convention-vue3
│   └── code-convention-vue3-patterns ── code-convention-security
└── code-convention-node
    └── code-convention-node-ops ── code-convention-security

code-convention-ios       (standalone, Swift)
code-convention-android   (standalone, Kotlin)
```
